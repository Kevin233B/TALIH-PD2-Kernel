...
#define write_cmos_sensor_8bit(...) subdrv_i2c_wr_u8(__VA_ARGS__)
...
#endif
  .iMirrorFlip = 0,	/* 0:Normal; 1:H; 2:V; 3HV (ctx->mirror - Normal)*/
};
#endif
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	sensor_id = (read_cmos_sensor(ctx, 0x03f0) << 8) | read_cmos_sensor(ctx, 0x03f1);
	return sensor_id;
}
...
static void set_max_framerate(struct subdrv_ctx *ctx, kal_uint16 framerate, kal_bool min_framelength_en)
{
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable)
		write_cmos_sensor_8bit(ctx, 0x0100, 0x01);
	else
		write_cmos_sensor_8bit(ctx, 0x0100, 0x00);
	return ERROR_NONE;
}
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	table_write_cmos_sensor(ctx, gc13a0_init_addr_data,
...
#if PDAFSUPPORT
	table_write_cmos_sensor(ctx, gc13a0_4208x3120_addr_data,
	  sizeof(gc13a0_4208x3120_addr_data)/sizeof(kal_uint16));
#else
	table_write_cmos_sensor(ctx, gc13a0_2104x1560_addr_data,
...
static void hs_video_setting(struct subdrv_ctx *ctx)
{
	...
	table_write_cmos_sensor(ctx, gc13a0_1280x720_addr_data,
...
static kal_uint32 set_test_pattern_mode(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable)
		write_cmos_sensor_8bit(ctx, 0x008c, 0x01);
	else
		write_cmos_sensor_8bit(ctx, 0x008c, 0x00);
 ...
 *
 *************************************************************************/
static kal_uint32 get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
 ...
 *
 *************************************************************************/
static kal_uint32 open(struct subdrv_ctx *ctx)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
 ...
 *
 *************************************************************************/
static kal_uint32 close(struct subdrv_ctx *ctx)
{
	pr_debug("E\n");
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	capture_setting(ctx->current_fps);
	set_mirror_flip(ctx, ctx->mirror);
				...
				MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ctx->autoflicker_en = KAL_FALSE;
	normal_video_setting(ctx->current_fps);
	set_mirror_flip(ctx, ctx->mirror);
...
static kal_uint32 get_info(struct subdrv_ctx *ctx, enum MSDK_SCENARIO_ID_ENUM scenario_id,
...
static kal_uint32 control(struct subdrv_ctx *ctx, enum MSDK_SCENARIO_ID_ENUM scenario_id,
...
static kal_uint32 feature_control(struct subdrv_ctx *ctx, MSDK_SENSOR_FEATURE_ENUM feature_id,
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_REGISTER:
		write_cmos_sensor_8bit(ctx, sensor_reg_data->RegAddr,
...
const struct subdrv_entry tsppkcf3290_gc13a0_mipi_raw_entry = {
	.name = "tsppkcf3290_gc13a0_mipi_raw",
	.id = GC13A0_SENSOR_ID,
	.pw_seq = pw_seq,
