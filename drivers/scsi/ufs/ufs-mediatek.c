...
/* MTK "biolog" (block IO tracer) is a perf-tracing feature and is not required
 * for boot. Its implementation lives in the blocktag framework
 * (drivers/misc/mediatek/blocktag), which is not ported yet, so provide the
 * same no-op stubs that <mt-plat/mtk_blocktag.h> emits when
 * CONFIG_MTK_BLOCK_IO_TRACER is off.
 */
#if IS_ENABLED(CONFIG_MTK_BLOCK_IO_TRACER)
#error "CONFIG_MTK_BLOCK_IO_TRACER depends on the blocktag framework (not ported)"
#else
