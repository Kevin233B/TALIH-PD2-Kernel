...
static bool dummy_job_wa_in_range(const u8 *base, const u8 *end, off_t off, size_t sz)
{
...
int kbase_dummy_job_wa_load(struct kbase_device *kbdev)
{
	...
	if (!dummy_job_wa_in_range(fw, fw_end, 0, sizeof(*header))) {
	...
	if (!dummy_job_wa_in_range(fw, fw_end, header->info_offset, sizeof(*v2_info))) {
	...
	while (blob_offset) {
		...
		if (!dummy_job_wa_in_range(fw, fw_end, blob_offset, sizeof(*blob))) {
		...
		if (!dummy_job_wa_in_range(fw, fw_end, blob->payload_offset, blob->size)) {
